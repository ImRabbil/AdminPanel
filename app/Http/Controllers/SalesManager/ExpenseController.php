...
class ExpenseController extends Controller
{
...

    {
        ...
        // print_r($today);
        // exit();
        return view('sales.today_expense',compact('today'));

    }
    public function EditTodayExpense($id)
    {
        $today = DB::table('expenses')->where('id',$id)->first();
        return view('sales.edit_today_expense',compact('today'));
       ...
       $data['year']=$request->year;
        ...
        $exp = DB::table('expenses')->where('id',$id)->update($data);
                if($exp){
                    ...
                    return Redirect()->route('today_expense');
                }else{
